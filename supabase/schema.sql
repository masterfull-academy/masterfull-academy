...
create table if not exists public.results (
  ...
  created_at timestamptz not null default now(),
  constraint results_attempt_check check (attempt >= 1),
  constraint results_score_check check (score >= 0 and score <= 20),
  constraint results_correct_check check (correct >= 0),
  constraint results_total_check check (total > 0),
  constraint results_seconds_used_check check (seconds_used >= 0),
  constraint results_student_exam_attempt_unique unique (student_id, exam_id, attempt)
);
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'results_submission_id_key' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_submission_id_key unique (submission_id);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_student_exam_attempt_unique' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_student_exam_attempt_unique unique (student_id, exam_id, attempt);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_attempt_check' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_attempt_check check (attempt >= 1);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_score_check' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_score_check check (score >= 0 and score <= 20);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_correct_check' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_correct_check check (correct >= 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_total_check' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_total_check check (total > 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'results_seconds_used_check' and conrelid = 'public.results'::regclass)
  then alter table public.results add constraint results_seconds_used_check check (seconds_used >= 0);
  end if;
end $$;
